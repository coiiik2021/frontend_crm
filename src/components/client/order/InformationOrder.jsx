  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const resCodeCompany = await GetAllNameCountry();
        console.log("resCodeCompany", resCodeCompany);

        // Làm phẳng mảng nếu có mảng lồng
        const flatList = resCodeCompany
        ...
        flatList.forEach((entry) => {
          const name = entry.trim(); // Ví dụ: "Algeria (DZ)" hoặc "Cambodia"
          const match = name.match(/\(([A-Z]{2})\)/); // Lấy mã: DZ
          const countryCode = match ? match[1] : "";

          // Tránh trùng nếu tên đầy đủ đã có rồi (giữ nguyên "Algeria (DZ)")
          if (!countryDataMap[name]) {
            countryDataMap[name] = { countryCode }; // Dùng name đầy đủ làm key
            options.push({
              value: name, // full name, ví dụ: "Algeria (DZ)"
              label: name,
            });
          }
        });
      ...
      {showSavedPopup && (
        <div className="absolute right-0 top-12 z-50 bg-white dark:bg-gray-800 border rounded-lg shadow-lg w-full max-w-md p-4 overflow-y-auto max-h-96">
          ...
          <div className="space-y-3">
            {savedList.length === 0 && (
              <p className="text-sm">Chưa có thông tin nào.</p>
            )}
            {savedList.map((item, idx) => (
              ...
              >
                ...
                <div className="mt-2 flex gap-2">
                  <Button size="sm" onClick={() => handleSelectSaved(idx)}>
                    Chọn
                  </Button>
                  <Button
                    size="sm"
