...
const Products = ({
  ...
  const revertProduct = (apiProduct) => {
    const quantity = Number(apiProduct.quantity) || 0;
    const price = Number(apiProduct.price) || 0;
    const total = quantity * price;

    return {
      id: apiProduct.id,
      "Mô tả sản phẩm": apiProduct.description || "",
      "Xuất xứ": apiProduct.origin || "",
      "Số lượng": String(quantity),
      "Kiểu đơn vị": apiProduct.unit || "",
      "Giá trên 1 sản phẩm": String(price),
      "Giá Trị": total,
      totalPrice: total,
    };
  };
        ...
        if (field === "Số lượng" || field === "Giá trên 1 sản phẩm") {
          ...
          const price =
            field === "Giá trên 1 sản phẩm"
              ? Number(value)
              : Number(pkg["Giá trên 1 sản phẩm"]);
  ...
  const handleImportExcel = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      setProductsTotal({
        ...
        priceProduct: importedProducts.reduce(
          (sum, p) => sum + p.totalPrice,
          productsTotal.priceProduct
        ),
      });
    };
  ...
  const isProductDuplicate = (product) => {
    return favorites.some(
      (fav) =>
        fav["Mô tả sản phẩm"].toLowerCase() ===
          product["Mô tả sản phẩm"].toLowerCase() &&
  ...
  const saveToFavorites = (product) => {
    ...
    const converted = {
      id: "",
      name: "",
      quantity: Number(product["Số lượng"]) || 0,
      origin: product["Xuất xứ"] || "",
      unit: product["Kiểu đơn vị"] || "",
      hsCode: "",
      uses: "",
      price: Number(product["Giá trên 1 sản phẩm"]) || 0,
  ...
  const fetchProducts = async () => {
    try {
      const apiProducts = await GetAllProductFavorite(); // <- Đợi dữ liệu trả về
      console.log("apiProducts Test:", apiProducts);

      if (Array.isArray(apiProducts)) {
        const savedProducts = apiProducts.map(revertProduct);
        setFavorites(savedProducts);
      } else {
        console.warn("Dữ liệu trả về không phải mảng:", apiProducts);
      }
    } catch (error) {
      console.error("Lỗi khi gọi GetAllProductFavorite:", error);
    }
  };
  useEffect(() => {
    fetchProducts(); // Gọi hàm async bên trong useEffect
  }, []);
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between cursor-pointer mb-2">
          <div
            ...
            onClick={() => setIsOpenFormPackage(!isOpenFormPackage)}
          >
            <h2 className="text-xl sm:text-2xl font-bold text-purple-700 mr-2">
              Danh sách sản phẩm
            </h2>
            <motion.span
                  ...
                  filteredFavorites.map((fav) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-600 dark:text-gray-300">
                        Xuất xứ: {fav["Xuất xứ"]}
                        <br />
                        Số lượng: {fav["Số lượng"]} {fav["Kiểu đơn vị"]}
                        <br />
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-12 gap-3 sm:gap-5 items-end">
                    {[
                      ...
                      if (field === "Tên") {
                        ...
                        ["Số lượng", "Giá trên 1 sản phẩm", "Giá Trị"].includes(
                          field
                        )
                      ) {
                        ...
                        <div key={field} className={columnClass}>
                          ...
                          {field === "Mô tả sản phẩm" ? (
                            <textarea
                              ...
                              className={`w-full border rounded px-3 py-2 transition-all duration-300 resize-y min-h-[80px] ${
            ...
            <div className="flex flex-col items-center">
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={1.5}
                  d="M8 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM15.5 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z"
                />
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
