...
export default function AWBContent({ bill_id }: { bill_id: string }) {
  ...
  useEffect(() => {
    const fetchUploadedFiles = async () => {
      try {
        const response = await axios.post("http://localhost:8080/api/files/get", {
          bill_id,
          type: "pdf",
        });
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
      <div className="relative w-full mb-5">
        <form>
          <div className="flex items-center gap-2">
            <button type="button" className="flex items-center justify-center w-10 h-10 bg-gray-200 rounded-full">
              <svg
            ...
            <input
              ...
              className="flex-1 h-10 rounded-lg border border-gray-300 px-4 text-sm placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
            />
          </div>
        </form>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col items-center gap-3">
          <label
            htmlFor="fileInput"
            className="w-full cursor-pointer rounded-lg bg-blue-500 px-4 py-2 text-center text-white hover:bg-blue-600"
          >
            Chọn File
          </label>
          <input
          ...
          />
          {fileName && <div className="text-sm text-gray-700">File đã chọn: <strong>{fileName}</strong></div>}
        </div>
        ...
        <div className="mt-4 space-y-3">
          {filteredFiles.map((file) => (
            <div
              key={file.id}
              className="flex items-center justify-between rounded-lg bg-gray-100 p-3 cursor-pointer"
              onClick={() => handleOpenFile(file.url)}
            >
              <div>
                <p className="text-sm font-medium text-gray-800">{file.name}</p>
                <p className="text-xs text-gray-500">Ngày upload: {file.created_at}</p>
              </div>
              <button
                onClick={(e) => {
                  e.stopPropagation();
