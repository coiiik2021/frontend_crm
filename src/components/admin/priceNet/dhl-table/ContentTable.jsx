...
export default function ContentTable() {
    ...
    const [tableType, setTableType] = useState("priceECO");
    ...
    useEffect(() => {
        const fetchData = async () => {
            const dataECO = await GetPriceNet("dhl-eco");
            const dataVN = await GetPriceNet("dhl-vn");

            setPriceVN(dataVN);
            setPriceECO(dataECO);

            setDataByDate(dataECO);
            const firstDate = Object.keys(dataECO)[0];
    ...
    const handleSaveData = async () => {
        ...
        if (tableType === "priceVN") {
            await PostPriceNet("dhl-vn", dataByDate);
        } else {
            await PostPriceNet("dhl-eco", dataByDate);
        }
...

                {
                    ...
                    <>
                        ...
                        <select
                            ...
                            className="px-4 py-2 border rounded"
                        >
                            <option value="priceECO">Giá ECO</option>
                            <option value="priceVN">Giá VN</option>
                        </select>
